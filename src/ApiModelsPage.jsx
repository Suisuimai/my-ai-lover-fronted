...
function RouteCard({ feature, route, connections, routes, catalogs, onLoadModels, onSave, busy }) {
...
export default function ApiModelsPage({ open, onClose }) {
  ...
  return <div className="api-page">
    ...
    <main className="api-page-main">
      ...
      {tab === "routes" && <section><div className="api-section-heading"><div><h2>功能分配</h2><p>每项工作同时记住连接和完整模型 ID。可读取模型目录、搜索选择，也始终允许手工填写。</p></div></div>{!connections.length ? <div className="api-empty"><p>请先添加至少一条连接。</p><button className="api-button dark" onClick={() => setTab("connections")}>去添加连接</button></div> : <div className="api-route-list">{FEATURES.map((feature) => { const route = routeMap.get(feature.id); return <RouteCard key={`${feature.id}-${route?.updatedAt || "new"}-${connections.length}`} feature={feature} route={route} routes={routes} connections={connections.filter((item) => item.enabled)} catalogs={catalogs} onLoadModels={loadModels} busy={busy === feature.id} onSave={saveRoute} />; })}</div>}</section>}
