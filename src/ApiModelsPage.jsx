...
const PRESETS = {
  openrouter: { label: "主力 OpenRouter", baseUrl: "https://openrouter.ai/api/v1", apiFormat: "openai_compatible" },
...
const FEATURES = [
  { id: "companion_chat", name: "伴侣聊天", description: "生成每次正式回复", direct: true },
  { id: "long_term_memory_extraction", name: "记忆整理", description: "提取长期事实与经历", direct: true },
  { id: "timeline_generation", name: "候选日记", description: "整理导入的 Claude 对话", direct: true },
  { id: "conversation_title", name: "窗口标题", description: "默认跟随伴侣聊天", follows: "companion_chat" },
  { id: "conversation_summary", name: "窗口摘要", description: "默认跟随记忆整理", follows: "long_term_memory_extraction" },
  { id: "followup_interpretation", name: "Followup 判断", description: "默认跟随记忆整理", follows: "long_term_memory_extraction" },
];
...
function ConnectionCard({ connection, onEdit, onDelete, busy }) {
  return <article className="api-card">
    ...
    <div className="api-actions"><button className="api-button ghost" onClick={() => onEdit(connection)}>编辑</button><button className="api-button danger" disabled={busy} onClick={() => onDelete(connection)}>删除</button></div>
  </article>;
}

function RouteCard({ feature, route, connections, routes, onSave, busy }) {
  ...
  return <article className="api-route-card">
    <div className="api-route-copy"><strong>{feature.name}</strong><span>{feature.description}</span></div>
    ...
    {following ? <div className="api-follow-value">{source ? `${source.connectionLabel} · ${source.modelId}` : "请先配置被跟随的功能"}</div> : <div className="api-route-fields"><select value={connectionId} onChange={(event) => setConnectionId(event.target.value)}>{connections.map((connection) => <option key={connection.id} value={connection.id}>{connection.label}</option>)}</select><input value={modelId} onChange={(event) => setModelId(event.target.value)} placeholder="完整模型 ID，例如 deepseek-v4-flash" /></div>}
...
export default function ApiModelsPage({ open, onClose }) {
  ...
  const load = useCallback(async () => {
    const [connectionData, routeData] = await Promise.all([api("/api-connections"), api("/api-feature-routes")]);
  ...
  useEffect(() => {
    if (!open) return;
    let active = true;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- load server state when this full-page panel opens.
