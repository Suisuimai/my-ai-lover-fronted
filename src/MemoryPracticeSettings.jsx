...
export default function MemoryPracticeSettings(){
  ...
  async function extract(segment){try{setBusy(segment.id);setMessage("便宜模型正在提取经历、证据与文档素材；不合格的证据格会自动重试一次。 ");await api(`/memory-practice/segments/${segment.id}/extract`,{method:"POST"});await refresh();setMessage("提取完成。结果只保存在候选区，尚未进入正式记忆。 ")}catch(error){setMessage(error.message)}finally{setBusy("")}}
